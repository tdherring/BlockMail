...
class Client:
    def __init__(self, host, port, data):
        ...
        thread = threading.Thread(target=self.establishSocket, args=(host, port, data)) #Create client thread to avoid blocking whilst waiting for reponse from other known nodes.
    ...
    def establishSocket(self, host, port, data):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            try:
                s.connect((host, port))
                print(host + ":" + str(port) + " - Connected! Sending Transaction...")
            #If connection to node failed, log, and wait 10 seconds to retry.
            except ConnectionRefusedError:
                if self.__timeout_counter >= 10:
                    print(host + ":" + str(port) + " - Connection refused (node may be offline). Connection Timed Out (" + self.__timeout_counter + ")")
                print(host + ":" + str(port) + " - Connection refused (node may be offline). Retrying in 10 seconds... (" + self.__timeout_counter + ")")
                time.sleep(10) 
