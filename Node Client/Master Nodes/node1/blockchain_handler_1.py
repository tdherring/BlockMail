...
class Server():
        ...
        with connection:
            while True:
                ...
                if data:
                    ...
                    if os.stat(self._tmp_file_name).st_size == expected_size:  # All data in stream received?
    ...
    def processIncomingData(self, data):
        ...
        while not file_name_set:
            try:
                self._tmp_file_name = f"{self.__server_type + str(file_num)}.tmp"
                tmp_file = open(self._tmp_file_name, "a+")
                file_name_set = True
            except:
                file_num += 1
                self._tmp_file_name = f"{self.__server_type + str(file_num)}.tmp"
...
class DiscoverySever(Server):

    def __init__(self):
        super(DiscoverySever, self).__init__(port=DISCOVERY_PORT, server_type="DISCOVERY")
    ...
    def nodeCommunicationControl(self):
        json_data = json.loads(open(self._tmp_file_name, "r").read())
        os.remove(self._tmp_file_name)  # ... And remove the tmp file.
    ...
    def chooseNodesToAdd(self):
        ...
        if NODES_ON_NETWORK[node_index] not in KNOWN_NODES and NODES_ON_NETWORK[node_index] != SERVER_IP:
            ...
            node_client_thread = threading.Thread(target=DiscoveryClient, args=(node,))
...
class SyncServer(Server):
    def __init__(self):
        super(SyncServer, self).__init__(port=SYNC_PORT, server_type="BLOCKCHAIN")
        print("[DISCOVERY] Sync Server Started!")
        self._data_received.wait()
        self.checkBlockchainUpdate()

    def checkBlockchainUpdate(self):
        if os.stat(self._tmp_file_name).st_size > os.stat("blocks/blockchain.chain").st_size:
            print("[BLOCKCHAIN] Updating Blockchain...")
            shutil.copy(self._tmp_file_name, f"blocks/blockchain.chain")
...
class BlockServer(Server):
    ...
    def __init__(self):
        super(BlockServer, self).__init__(port=BLOCK_PORT, server_type="BLOCK")
        while not self._file_created.isSet():
            True
        print("[DISCOVERY] Block Server Started!")
        self._data_received.wait()
        self.checkBlockSize()

    def checkBlockSize(self):
        block_num = self.getBlockNum()
        if os.stat(self._tmp_file_name).st_size > os.stat(f"blocks/{block_num}.block").st_size:
            print("[BLOCK] Larger Block Received! Updating...")
            shutil.copy(self._tmp_file_name, f"blocks/{block_num}.block")
            print("[BLOCK] Blockchain Successfully Updated!")
        else:
            print("[BLOCK] Comparing Received Block...")
        os.remove(self._tmp_file_name)  # ... And remove the tmp file.

    def getBlockNum(self):
        for prefix, val_type, value in ijson.parse(open(self._tmp_file_name, "r")):
            if prefix == "block":
                return value
    ...
    def establishSocket(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:  # Open a new socket, s.
            try:
                ...
                print(f"[BLOCK] {self.__host}:{str(self.__port)} - Connection refused (node may be offline). Retrying in 10 seconds...")
                time.sleep(10)  # Wait for 10 seconds
                self.establishSocket()  # Retry
...
if __name__ == "__main__":
    ...
    for node in MASTER_NODES:
        if node != SERVER_IP:
            ...
            sync_client_thread.start()
            block_client_thread.start()
    node_server_thread = threading.Thread(target=DiscoverySever)
    sync_server_thread = threading.Thread(target=SyncServer)
    block_server_thread = threading.Thread(target=BlockServer)
