...
class Client:
    def __init__(self, host, port):
        thread = threading.Thread(target=self.establishSocket, args=(host, port))
        thread.start()
...
server = Server("localhost", 50430)
server = Server("localhost", 50431)
server = Server("localhost", 50432)
server = Server("localhost", 50433)
server = Server("localhost", 50434)
server = Server("localhost", 50435)
known_nodes = GeneralFunctions.readKnownNodes()
